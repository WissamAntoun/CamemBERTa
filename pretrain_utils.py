...
def get_dataset(
    config: DebertaV3PretrainingConfig, batch_size, is_training, input_pipeline_context
):
    ...
    for input_pattern in config.pretrain_tfrecords.split(","):
        input_tf_records = os.path.join(config.data_prep_working_dir, input_pattern)
        input_files.extend(tf.io.gfile.glob(input_tf_records))
        logging.info("input_files pattern: %s", input_tf_records)

    d = tf.data.Dataset.from_tensor_slices(tf.constant(input_files))
    if config.use_horovod:
        d = d.shard(num_shards=hvd.size(), index=hvd.rank())
    if input_pipeline_context and input_pipeline_context.num_input_pipelines > 1:
        logging.info("Using input pipeline context to shuffle records")
        logging.info(
            "input_pipeline_context.num_input_pipelines: %d",
...
def get_pretrain_dataset_fn(config, global_batch_size, hvd=None):
    ...
    def _dataset_fn(ctx=None):
        ...
        logging.info("#### per_replica_batch_size = %d", batch_size)
        train_dataset = get_dataset(
            config, batch_size, is_training=True, input_pipeline_context=ctx
        )
...
def _get_candidates_mask(inputs: Inputs, ignore_ids, disallow_from_mask=None):
...
def mask(
    ...
    candidates_mask = _get_candidates_mask(
        inputs, config.ignore_ids_dict.values(), disallow_from_mask
    )
