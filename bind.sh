...
echo "num_sockets=${num_sockets} num_nodes=${num_nodes} cores_per_socket=${cores_per_socket} num_gpus=${num_gpus}"


readonly cores_per_node=$(( (num_sockets * cores_per_socket) / num_nodes ))
if [ ${num_gpus} -gt 1 ]; then
    numerator=$(( num_gpus + num_nodes - 1 ))
    readonly gpus_per_node=$(( numerator / num_nodes ))
    # readonly gpus_per_node=3
else
    readonly gpus_per_node=1
fi
echo "cores_per_node = ${cores_per_node} gpus_per_node=${gpus_per_node}"
...
case "${cluster}" in
    ...
    selene)
        ...
        ;;
    dellr7525gpu)
        echo "dellr7525gpu detected, setting ibdevs to mlx5_0"
        ibstat -l
        ibdevs=(mlx5_0)
        ;;
    '')
...
# ibdevs=(mlx5_0)
# num_ibdevs=1
echo "num_ibdevs=${num_ibdevs} ibdevs=${ibdevs[*]} ibstat_out=${ibstat_out}"
