...
"""Writes out text data as tfrecords that ELECTRA/DeBERTa can be pre-trained on."""
...
# for some reasong the fast tokenizer was actually way slower than the regular one,
# maybe this has changed with the updated transformers verison
# haven't tested than one yet
# also nvidia-dlogger and sentencepiece don't like each other, results in segfault

# from fast_tokenizer.tokenization_deberta_v2_fast import DebertaV2TokenizerFast
from fast_tokenizer.tokenization_deberta_v2 import DebertaV2Tokenizer
...
def main():
    ...
    print("Args: ", args)
    print("Starting pre-training dataset creation")
    if args.num_processes == 1:
        write_examples(0, args)
    else:
        print("Starting {} processes".format(args.num_processes))
        jobs = []
        for i in range(args.num_processes):
            job = multiprocessing.Process(target=write_examples, args=(i, args))
            jobs.append(job)
            job.start()
            print("Started job {}".format(i))
        for job in jobs:
            print("Joined job {}".format(job.pid))
            job.join()
    print("Done!")
