...
class TFRobertaEmbeddings(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        log("embedding_size: ", self.embedding_size)
        self.hidden_size = config.hidden_size
        log("hidden_size: ", self.hidden_size)
    ...
    def update_embeddings(self):
        ...
        with tf.name_scope("tf_roberta_model/roberta/embeddings/word_embeddings"):
            if self.disentangled_gradients:
        ...
        if self.type_vocab_size > 0:
            with tf.name_scope(
                "tf_roberta_model/roberta/embeddings/token_type_embeddings"
            ):
        ...
        if self.position_biased_input:
            with tf.name_scope(
                "tf_roberta_model/roberta/embeddings/position_embeddings"
            ):
...
class TFRobertaMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.embeddings = TFRobertaEmbeddings(
            config,
...
class TFRobertaForMaskedLM(TFRobertaPreTrainedModel, TFMaskedLanguageModelingLoss):
    ...
    def update_embeddings(self):
        self.roberta.embeddings.update_embeddings()
...
class TFRobertaDiscriminatorPredictions(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
...
@dataclass
class TFRobertaForRTDOutput(ModelOutput):
    """
    Output type of [`TFRobertaForPreTraining`].

    Args:
        loss (*optional*, returned when `labels` is provided, `tf.Tensor` of shape `(1,)`):
            Total loss of the Roberta RTD objective.
...
class TFRobertaForRTD(TFRobertaPreTrainedModel):
    def __init__(
        self,
        config,
        shared_embeddings=False,
        ...
        **kwargs,
        ...
        self.discriminator_predictions = TFRobertaDiscriminatorPredictions(
    ...
    @replace_return_docstrings(
        output_type=TFRobertaForRTDOutput, config_class=_CONFIG_FOR_DOC
    )
    def call(
        self,
        input_ids: Optional[TFModelInputType] = None,
...
def get_generator_config(config, bert_config):
    ...
    gen_config = RobertaConfig.from_dict(bert_config.to_dict())
...
class PretrainingModel(tf.keras.Model):
    ...
    def __init__(self, config: RobertaPretrainingConfig, **kwargs):
        super().__init__(**kwargs)
        # Set up model config
        self._config = config
        self.disc_config = RobertaConfig(
        ...
        if config.electra_objective:
            ...
            if config.shared_embeddings:
                ...
                self.discriminator = TFRobertaForRTD(
                    self.disc_config,
                    shared_embeddings=True,
                    input_embeddings=self.generator.roberta.get_input_embeddings(),
                    disentangled_gradients=config.disentangled_gradients,
                )
            else:
                self.discriminator = TFRobertaForRTD(self.disc_config)
        else:
            self.generator = TFRobertaForMaskedLM(self.disc_config)
