...
def setup_logger(config):
    ...
    if not config.json_summary:
        log_path = os.path.join(
            config.log_dir, "dllogger_rank{}.log".format(get_rank())
        )
    else:
        log_path = "{}_rank{}".format(config.json_summary, get_rank())
...
def postprocess_dllog(config):
    if not config.json_summary:
        log_path = os.path.join(config.log_dir, "dllogger_rank{}.log")
    else:
        log_path = str(config.json_summary) + "_rank{}"
    logfiles = [open(log_path.format(i), "r") for i in range(get_world_size())]

    if not config.json_summary:
        log_path = os.path.join(config.log_dir, "dllogger.log")
    else:
        log_path = str(config.json_summary)
...
def print_model_layers(model, indent=0):
    if isinstance(model, tf.keras.layers.Layer):
        ...
        for var_k, var_v in vars(model).items():
            if (
                isinstance(var_v, tf.keras.layers.Layer)
                or str(type(var_v))
                == "<class 'tensorflow.python.training.tracking.data_structures.ListWrapper'>"
            ):
                log("   " * indent + "└─", var_k)
                print_model_layers(var_v, indent + 1)
            if isinstance(var_v, tf.Variable) and len(var_v.shape) > 0:
                log("   " * indent + "└─", var_k, var_v.shape)
            if var_k == "units":
                log("   " * indent + "└─", var_k, var_v)
    elif (
        str(type(model))
        == "<class 'tensorflow.python.training.tracking.data_structures.ListWrapper'>"
    ):
        for id, var_v in enumerate(model):
            if (
                isinstance(var_v, tf.keras.layers.Layer)
                or str(type(var_v))
                == "<class 'tensorflow.python.training.tracking.data_structures.ListWrapper'>"
            ):
                log("   " * indent + "└─", var_v.name)
                print_model_layers(var_v, indent + 1)
    else:
        return
