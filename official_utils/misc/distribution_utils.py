...
from official_utils.misc import tpu_lib


def _collective_implementation(all_reduce_alg):
    """Return a CommunicationImplementation based on all_reduce_alg.
    ...
    Returns:
      tf.distribute.experimental.CommunicationImplementation object
...
def _mirrored_cross_device_ops(all_reduce_alg, num_packs):
    ...
    mirrored_all_reduce_options = {
        ...
        "reduce_to_onedevice": tf.distribute.ReductionToOneDevice,
    }
    if all_reduce_alg not in mirrored_all_reduce_options:
        raise ValueError(
            "When used with `mirrored`, valid values for all_reduce_alg are "
            "['nccl', 'hierarchical_copy', 'reduce_to_onedevice'].  Supplied value: {}".format(
                all_reduce_alg
            )
        )
    cross_device_ops_class = mirrored_all_reduce_options[all_reduce_alg]
    if all_reduce_alg == "reduce_to_onedevice":
...
def get_distribution_strategy(
    ...
    if distribution_strategy == "multi_worker_mirrored":
        return tf.distribute.MultiWorkerMirroredStrategy(
            communication=_collective_implementation(all_reduce_alg)
        )
...
def set_up_synthetic_data():
    ...
    _monkey_patch_dataset_method(tf.distribute.MultiWorkerMirroredStrategy)


def undo_set_up_synthetic_data():
    _undo_monkey_patch_dataset_method(tf.distribute.OneDeviceStrategy)
    _undo_monkey_patch_dataset_method(tf.distribute.MirroredStrategy)
    _undo_monkey_patch_dataset_method(tf.distribute.MultiWorkerMirroredStrategy)
