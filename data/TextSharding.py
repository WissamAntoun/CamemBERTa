...
class Sharding:
    ...
    def load_articles(self):
        ...
        log("End: Loading Articles: There are", len(self.articles), "articles.")

    def load_articles_from_sentence_segmented_files(self):
        log("Start: Loading Sentences")
            ...
            with open(input_file, mode="r", newline="\n") as f:
                ...
                for i, line in enumerate(f):
                    if line.rstrip() == "" or line == "\n":
                        self.articles[global_article_count] = ""
                        self.sentences[global_article_count] = tmp_article_sentences
                        global_article_count += 1
                        global_sentence_count += len(tmp_article_sentences)
                        tmp_article_sentences = []
                    else:
                        tmp_article_sentences.append(line.rstrip())

        log("End: Loading Articles: There are %s articles." % len(self.articles))
        log("End: Loading Sentences: There are %s sentences." % global_sentence_count)

    def segment_articles_into_sentences(self, segmenter):
        log("Start: Sentence Segmentation")
    ...
    def distribute_articles_over_shards(self):
        log("Start: Distribute Articles Over Shards")
        assert (
        ...
        if len(unused_article_set) != 0:
            log("Warning: Some articles did not make it into output files.")
        ...
        log("End: Distribute Articles Over Shards")

    def distribute_articles_over_shards_single_set_only(self):
        log("Start: Distribute Articles Over single set of Shards only")
        assert (
            len(self.articles) >= self.n_training_shards
        ), "There are fewer articles than shards. Please add more data or reduce the number of shards requested."
        ...
        nominal_sentences_per_training_shard = total_sentences // self.n_training_shards
    ...
    def write_shards_to_disk(self):
        log("Start: Write Shards to Disk")
        for shard in self.output_training_files:
...
class SplitSharding:
    def __init__(
        ...
        assert (
            len(input_train_files) > 0
        ), "The input file list must contain at least one file."
        assert (
            len(input_test_files) > 0
        ), "The input file list must contain at least one file."
        assert (
            len(input_val_files) > 0
        ), "The input file list must contain at least one file."
        assert n_training_shards > 0, "There must be at least one output shard."
        assert n_test_shards > 0, "There must be at least one output shard."

        assert (
            n_training_shards < 9999 and n_test_shards < 9999 and n_val_shards < 9999
        ), "There must be at most 9999 output shards. Or you need to change the splitting script."
    ...
    def shard_files(self):
        ...
        for input_file in self.input_train_files:
            log("Start: Sharding %s" % input_file)
            input_file_name = input_file.split("/")[-1]
            SHARD_CMD = f"split -da 4 -n l/{self.n_training_shards} {input_file} {self.output_name_prefix}_{input_file_name}_training_ --additional-suffix={self.output_file_extension}"
            subprocess.call(SHARD_CMD, shell=True)

            log("End: Sharding %s" % input_file)

        for input_file in self.input_test_files:
            log("Start: Sharding %s" % input_file)
            input_file_name = input_file.split("/")[-1]
            SHARD_CMD = f"split -da 4 -n l/{self.n_test_shards} {input_file} {self.output_name_prefix}_{input_file_name}_test_ --additional-suffix={self.output_file_extension}"
            subprocess.call(SHARD_CMD, shell=True)
        ...
        for input_file in self.input_test_files:
            log("Start: Sharding %s" % input_file)
            input_file_name = input_file.split("/")[-1]
            SHARD_CMD = f"split -da 4 -n l/{self.n_test_shards} {input_file} {self.output_name_prefix}_{input_file_name}_val_ --additional-suffix={self.output_file_extension}"
            subprocess.call(SHARD_CMD, shell=True)

            log("End: Sharding %s" % input_file)


# split -da 4 -n l/1000 valid.fr part --additional-suffix=".txt"
